#!/usr/bin/python
# Author: Anton Mitrokhin, UMD 2016-2018
...
import getpass
# Check if we have ROS and the version
ROS_LIB_ROOT_DIR = "/opt/ros/"
ROS_INSTALL_DIR = ""
distributions = []
chosen_distribution = ""
...
GENERATOR_MARKER = ' # Generated by ROS-WS-INSTALL-PY' 
DEPS = []
...
def find_ros():
    global ROS_INSTALL_DIR
    global distributions
    global chosen_distribution

    try:
        distributions = os.listdir(ROS_LIB_ROOT_DIR)
    except:
        print bld(err("Unable to locate '" + ROS_LIB_ROOT_DIR + "' folder. Have you installed ROS?"))
        exit(1) 

    if (len(distributions) == 0):
        print bld(err("The '" + ROS_LIB_ROOT_DIR + "' folder is empty. Have you installed ROS?"))
        exit(1) 

    print bld(okb("Found ROS distributions:"))
    for distribution in distributions:
        print bld(wht("    " + distribution))

    if (len(distributions) > 1):
        print bld(wrn("Found more than one ROS distrbution. Choosing the first one..."))

    chosen_distribution = distributions[0]
    ROS_INSTALL_DIR = ROS_LIB_ROOT_DIR + chosen_distribution


def print_info():
    print bld(wht("\nDEPLOY SCRIPT FOR LISA PROJECT"))
    print bld(wht("author: Anton Mitrokhin, 2018"))
    print bld(wrn("Warning! "))+wht("This script assumes you had installed ROS to the ")+okb("'"+ROS_LIB_ROOT_DIR+"'")
    find_ros()
    print wht("")
...
def create_startup_scripts():
    ...
    "echo \"============================\" >> $LOGFILE\n"+
    "echo \"\" >> $LOGFILE\n"+
    "echo \"$(roslaunch lisa_bringup minimal.launch &>> $LOGFILE )\"\n")
...
parser = argparse.ArgumentParser(description='Deploy script for the Lisa project.')
...
gen_launcher('lisa-rebuild',           'lisaRebuild',         'rosRebuild.png',    'cd ' + ROOT_DIR + '\n\n'\
                                                                                           'catkin_make\n\n'\
                                                                                           'source ' + LOCAL_BASH_FILE + '\n'\
                                                                                           'echo "Press any key to continue..."\nread')

gen_launcher('lisa-rebuild-eclipse','lisaRebuild4Eclipse', 'rosRebuild4Eclipse.png', 'cd ' + ROOT_DIR + '\n'\
...
gen_launcher('lisa-core-4-real',       'lisaCore4Real',       'rosRun4Real.png',   'roslaunch lisa_demos core.launch')
